...
export default function TrainerApplicationStatusPage() {
  ...
  return (
    <ProtectedPage
      ...
      description="Проверка модерации, dashboard unlock и следующие шаги после approval."
    >
      <TrainerDashboardShell
        title="Статус заявки тренера"
        description="Здесь видно, почему dashboard открыт или заблокирован, и какое действие следующее."
      >
          ...
          <div className="stack" style={{ gap: 24 }}>
            <section className="grid-3">
              <div className="card stat-card">
                ...
                <strong>{state.application.status}</strong>
                <small>{state.application.reviewed_at || state.application.submitted_at || 'not submitted'}</small>
              </div>
              <div className="card stat-card">
                <span className="stat-label">Dashboard</span>
                <strong>{state.dashboard_unlocked ? 'Unlocked' : 'Locked'}</strong>
                <small>{state.summary.next_step_title}</small>
              </div>
              <div className="card stat-card">
                <span className="stat-label">Progress</span>
                <strong>{state.summary.completion_percent}%</strong>
                <small>{state.summary.completed_steps}/{state.summary.total_steps} steps</small>
              </div>
            </section>
            ...
            <section className="card stack" style={{ gap: 14 }}>
              <div className="row">
                <div>
                  <span className={`badge ${statusTone(state.application.status)}`}>{state.application.status}</span>
                  <h2 className="title-md" style={{ marginTop: 8 }}>Moderation result</h2>
                </div>
              ...
              {state.dashboard_unlocked ? (
                ...
                <p className="muted">Trainer dashboard откроется после admin approval и синхронизации trainer profile.</p>
              )}
            </section>

            <section className="card stack" style={{ gap: 14 }}>
              <h2 className="title-md">Readiness chain</h2>
