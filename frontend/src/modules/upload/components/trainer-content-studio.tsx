...
import { TrainerContentCard } from '@/modules/upload/components/trainer-content-card';
import { TrainerVideoUploadCard } from '@/modules/upload/components/trainer-video-upload-card';
import {
  makeTrainerContentSlug,
  trainerContentEntityLabel,
  trainerContentPrice,
  trainerContentStatusLabel,
  trainerContentStatusTone,
  type TrainerContentStatusTone,
  type TrainerContentTab,
} from '@/modules/upload/components/trainer-upload-format';
...
function statusLabel(status?: string) {
  return trainerContentStatusLabel(status);
}

function statusTone(status?: string): TrainerContentStatusTone {
  return trainerContentStatusTone(status);
}

function resolveEntityLabel(tab: TrainerContentTab) {
  return trainerContentEntityLabel(tab);
}
...
function formatPrice(value?: string, currency?: string) {
  return trainerContentPrice(value, currency);
}
...
export function TrainerContentStudio() {
  ...
  useEffect(() => {
    ...
    if (queryTab === 'videos' || queryTab === 'programs' || queryTab === 'bundles') {
      setTab(queryTab);
    }
    if (queryIntent === 'upload') {
      setTab('videos');
  ...
  function renderContentCard(entity: StudioEntity, entityTab: TrainerContentTab) {
    ...
    return (
      <TrainerContentCard
        ...
        actions={
          <>
          <button className="trainer-content-button-secondary" type="button" onClick={() => {
          ...
          {entityTab === 'videos' ? (
            <button className="trainer-content-button-secondary" type="button" onClick={() => void submitVideoForReview(entity.id)} disabled={actionId === entity.id}>
              Отправить на проверку
            </button>
          ) : null}
          <button
            className="trainer-content-button"
            type="button"
            ...
            <div className="trainer-program-editor">
              <form className="trainer-content-form" onSubmit={saveProgram}>
                <div className="trainer-content-editor-header">
                  ...
                  <button className="trainer-content-button-secondary" type="button" onClick={() => startNew('programs')} disabled={saving || Boolean(actionId)}>Новая программа</button>
                </div>
                <label className="trainer-content-field"><span>Название программы</span><input className="trainer-content-input" value={programForm.title} onChange={(event) => setProgramForm((current) => ({ ...current, title: event.target.value, slug: current.slug || makeTrainerContentSlug(event.target.value) }))} required /></label>
                <label className="trainer-content-field"><span>Публичный адрес</span><input className="trainer-content-input" value={programForm.slug} onChange={(event) => setProgramForm((current) => ({ ...current, slug: makeTrainerContentSlug(event.target.value) }))} required /></label>
                <label className="trainer-content-field"><span>Описание</span><textarea className="trainer-content-textarea" rows={4} value={programForm.description} onChange={(event) => setProgramForm((current) => ({ ...current, description: event.target.value }))} /></label>
                <div className="trainer-content-form-grid">
                  <label className="trainer-content-field"><span>Цена</span><input className="trainer-content-input" type="number" min="0" step="0.01" value={programForm.price_amount} onChange={(event) => setProgramForm((current) => ({ ...current, price_amount: event.target.value }))} /></label>
                  <label className="trainer-content-field"><span>Валюта</span><input className="trainer-content-input" value={programForm.currency} onChange={(event) => setProgramForm((current) => ({ ...current, currency: event.target.value.toUpperCase() }))} /></label>
                </div>
                <div className="trainer-content-actions">
                  <button className="trainer-content-button" type="submit" disabled={saving}>{selectedProgramId ? 'Обновить программу' : 'Сохранить программу'}</button>
                  {selectedProgramId ? <button className="trainer-content-button-secondary" type="button" onClick={() => void publishAndComplete(() => uploadApi.publishProgramDraft(selectedProgramId), selectedProgramId, 'programs')} disabled={saving || Boolean(actionId)}>Опубликовать программу</button> : null}
                </div>
              </form>
              ...
              <div className="trainer-lesson-editor">
                <div className="trainer-content-editor-header">
                  ...
                  <button className="trainer-content-button-secondary" type="button" onClick={() => { setSelectedLessonId(null); setLessonForm(initialLessonForm); }} disabled={!selectedProgramId}>Добавить урок</button>
                </div>
                <form className="trainer-content-form" onSubmit={saveLesson}>
                  ...
                  <div className="trainer-content-actions">
                    ...
                    <button className="trainer-content-button-secondary" type="button" onClick={() => void moveLesson(-1)} disabled={selectedLessonIndex <= 0 || Boolean(actionId)}>Выше</button>
                    <button className="trainer-content-button-secondary" type="button" onClick={() => void moveLesson(1)} disabled={selectedLessonIndex < 0 || selectedLessonIndex >= sortedProgramLessons.length - 1 || Boolean(actionId)}>Ниже</button>
                  </div>
                </form>
            ...
            <div className="trainer-bundle-editor">
              <form className="trainer-content-form" onSubmit={saveBundle}>
                <div className="trainer-content-editor-header">
                  ...
                  <button className="trainer-content-button-secondary" type="button" onClick={() => startNew('bundles')} disabled={saving || Boolean(actionId)}>Новый набор</button>
                </div>
                <label className="trainer-content-field"><span>Название набора</span><input className="trainer-content-input" value={bundleForm.title} onChange={(event) => setBundleForm((current) => ({ ...current, title: event.target.value, slug: current.slug || makeTrainerContentSlug(event.target.value) }))} required /></label>
                <label className="trainer-content-field"><span>Публичный адрес</span><input className="trainer-content-input" value={bundleForm.slug} onChange={(event) => setBundleForm((current) => ({ ...current, slug: makeTrainerContentSlug(event.target.value) }))} required /></label>
                <label className="trainer-content-field"><span>Описание</span><textarea className="trainer-content-textarea" rows={4} value={bundleForm.description} onChange={(event) => setBundleForm((current) => ({ ...current, description: event.target.value }))} /></label>
                <div className="trainer-content-form-grid">
                  <label className="trainer-content-field"><span>Цена</span><input className="trainer-content-input" type="number" min="0" step="0.01" value={bundleForm.price_amount} onChange={(event) => setBundleForm((current) => ({ ...current, price_amount: event.target.value }))} /></label>
                  <label className="trainer-content-field"><span>Валюта</span><input className="trainer-content-input" value={bundleForm.currency} onChange={(event) => setBundleForm((current) => ({ ...current, currency: event.target.value.toUpperCase() }))} /></label>
                </div>
                <div className="trainer-content-actions">
                  <button className="trainer-content-button" type="submit" disabled={saving}>{selectedBundleId ? 'Сохранить набор' : 'Новый набор'}</button>
                  {selectedBundleId ? <button className="trainer-content-button-secondary" type="button" onClick={() => void publishAndComplete(() => uploadApi.publishBundleDraft(selectedBundleId), selectedBundleId, 'bundles')} disabled={saving || Boolean(actionId)}>Опубликовать набор</button> : null}
                </div>
              </form>
              ...
              <form className="trainer-content-form" onSubmit={saveBundleItem}>
                ...
                <div className="trainer-content-actions">
                  ...
                  <button className="trainer-content-button-secondary" type="button" onClick={() => void deleteBundleItem()} disabled={!selectedBundleItemId || actionId === selectedBundleItemId}>Удалить материал</button>
                  <button className="trainer-content-button-secondary" type="button" onClick={() => void moveBundleItem(-1)} disabled={selectedBundleItemIndex <= 0 || Boolean(actionId)}>Выше</button>
                  <button className="trainer-content-button-secondary" type="button" onClick={() => void moveBundleItem(1)} disabled={selectedBundleItemIndex < 0 || selectedBundleItemIndex >= sortedBundleItems.length - 1 || Boolean(actionId)}>Ниже</button>
                </div>
              </form>
